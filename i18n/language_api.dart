...
extension LanguageApi on AppLanguage {
  ...
  Future<ResponseData<T>> getAllLabel<T extends RData>(OnFail onFail) async {
    ...
    var request = RequestTask.set(
      urlPath,
      headerType: HeaderType.standard
    );
    try {
      ResponseData<T> response =
          await requestFilter<T>(request, HttpMethod.get);
      return response;
    } catch (e) {
  ...
  Future<ResponseData<T>> getLabelWith<T extends RData>(
      String locale, OnFail onFail) async {
    ...
    try {
      ResponseData<T> response =
          await requestFilter<T>(request, HttpMethod.post);
      return response;
    } catch (e) {
      return ResponseData<T>(error: onObjectException(e, onFail));
    }
